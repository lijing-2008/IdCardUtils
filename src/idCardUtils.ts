...
export class IdCardUtils {
    ...
    getOriginObject(code: string):IOrigin {
        ...
        // 获取省
        // @ts-ignore
        const PItem = ChinaProvinceCity.find(
            (item) => item.code === code.substring(0, 2)
        )
        if (PItem && PItem.children) {
            ...
            // 获取市
            const CItem = PItem.children.find(
                (item) => item.code === code.substring(0, 4)
            )
    ...
    getOriginName(code: string):string {
        const origin: IOrigin = this.getOriginObject(code)
